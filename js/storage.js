...
/* The standard taxonomy for a domain: which categories exist, in the order
   the publisher numbers them, and which standard they come from. Generated by
   build-data.py from the cases themselves, so it can never list a category
   the data does not use or miss one that it does. */
function categoryIndexFor(domainCode){
  return categoryIndex[domainCode] || null;
}

/* A custom case is authored without a category, so it joins the domain's
   "uncategorised" bucket rather than silently disappearing from a picker that
   only lists standard categories. */
const UNCATEGORISED = { code: 'CUSTOM', name: 'Custom cases' };
...
function buildCustomCaseItem(input){
  ...
  const inDomain = allData.filter(d => d.domain === domain);
  const maxSeq = Math.max(0, ...inDomain.map(d => d.sequence || 0));
  const maxIdx = Math.max(0, ...inDomain.map(d => d.domainIndex || 0));
  const customsHere = inDomain.filter(d => d.custom).length;

  /* `sequence` is the global ordering key and, since build-data.py started
     deriving it, every domain owns one contiguous integer block. maxSeq + 1
     would therefore land on the FIRST case of the next domain and scramble
     both Testing Order sort and the "next untested" lookup. A fractional
     offset sorts the custom case immediately after its domain's last case
     and can never collide with another domain's integers — sequence is only
     ever compared, never displayed. */
  return {
    id: nextCustomId(domain),
    domain, custom: true,
    sequence: maxSeq + (customsHere + 1) / 1000,
    /* What the row badge actually shows: the next number in this domain. */
    domainIndex: maxIdx + 1,
    /* Custom cases are authored by the analyst, so no published standard
       classifies them. They get their own bucket, which the category picker
       only shows once at least one exists — mapping them into a real OWASP
       or NIST category would be an assertion nobody verified. */
    categoryStandard: (categoryIndexFor(domain) || {}).standard || '',
    categoryCode: 'CUSTOM',
    categoryName: 'Custom cases',
    /* Custom cases are authored by the analyst, not classified against a
       published standard, so they get their own bucket in the picker rather
       than being forced into a category they were never assessed against. */
    categoryStandard: (categoryIndexFor(domain) || {}).standard || '',
    categoryCode: UNCATEGORISED.code,
    categoryName: UNCATEGORISED.name,
    ...
    // Optional on custom cases: the generated corpus always carries one, but a
...
function applyProgress(payload){
  ...
  allData.forEach(d=>{
    ...
    if(notesMap[d.id]){
      let parsed = null;
      try{ parsed = JSON.parse(notesMap[d.id]); }
      catch(e){ parsed = { findings: String(notesMap[d.id]) }; }
      d.assessorNotes = normalizeAssessorNotes(parsed);
    }
  });
  if(payload.tester){
...
/* Imported progress files are attacker-controlled: they arrive as a file the
   user chose, and nothing about that file has been through our code before.
   Parsing it yields whatever shape the author wrote, so every field is forced
   back to its expected type here rather than trusted. Without this, a notes
   object whose evidenceLinks is a string (or a number, or null) crashes the
   detail renderer on .map(), and attachments could carry arbitrary values
   into the DOM. */
function normalizeAssessorNotes(raw){
  const src = (raw && typeof raw === 'object' && !Array.isArray(raw)) ? raw : {};
  const str = v => (typeof v === 'string' ? v : '');
  const strArray = v => (Array.isArray(v) ? v.filter(x => typeof x === 'string') : []);
  return {
    findings:          str(src.findings),
    pocDetails:        str(src.pocDetails),
    evidenceLinks:     strArray(src.evidenceLinks),
    affectedEndpoints: strArray(src.affectedEndpoints),
    /* Attachments keep only the two fields the UI reads, both as strings.
       renderAttachmentGrid() validates dataUrl again before it reaches a
       src attribute — this is the outer of two gates, not the only one. */
    attachments: (Array.isArray(src.attachments) ? src.attachments : [])
      .filter(a => a && typeof a === 'object')
      .slice(0, 8)
      .map(a => ({ name: str(a.name), dataUrl: str(a.dataUrl) })),
    remediation: normalizeRemediation(src.remediation),
    status: str(src.status) || 'not-tested',
  };
}
